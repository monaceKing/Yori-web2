...
export class AddCategorieComponent {
    ...
    filteredEquipements = this.equipements; // Equipements filtrés
    searchTerm = ''; // Terme de recherche
    onSearchChange(event: Event): void {
      const inputValue = (event.target as HTMLInputElement).value; // Cast explicite
      this.searchTerm = inputValue.toLowerCase();
      this.filteredEquipements = this.equipements.filter(equipement =>
        equipement.label.toLowerCase().includes(this.searchTerm)
      );
    }
